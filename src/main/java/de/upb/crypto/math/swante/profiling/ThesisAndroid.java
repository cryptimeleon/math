...
public class ThesisAndroid {
    public static void main(String[] args) {
        pln("=========================");
        if (args.length == 0) {
            args = "10 100 1 1 True".split(" ");
        }
        pln(args);
        int bitLength = Integer.parseInt(args[0]);
        MyShortFormWeierstrassCurveParameters parameters = MyUtil.createBnSFC256WeierstrassCurveGroupParams();
        ...
        int numBases = Integer.parseInt(args[0]);
        int numIterations = Integer.parseInt(args[1]);
        int windowSize = Integer.parseInt(args[2]);
        int m = (1 << windowSize) - 1;
        int algo = Integer.parseInt(args[3]);
        boolean cacheSmallPowers = args[4].equals("True");
        ...
        for (int iter = -numIterations; iter < numIterations; iter++) {
            ...
            if (System.nanoTime() / 1.0e6 - originalStartMillis > 15*1000) { // exit early if this run will take too long
                pln("__result__: too much");
                return;
            }
        }
        double elapsedMillis = System.nanoTime() / 1.0e6 - startMillis;
        pln("__result__: " + String.join(" ", args) + " " + elapsedMillis);
    }
}
